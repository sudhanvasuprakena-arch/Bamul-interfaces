...
CREATE OR REPLACE PACKAGE BODY apps.xxcust_om_gl_interface_pkg AS
    ...
    EXCEPTION
        WHEN OTHERS THEN NULL; -- Ignore buffer overflow
    END log_msg;
    ...
    -- =========================================================================
    -- Helper: Resolve an active apps user for apps_initialize
    -- =========================================================================
    FUNCTION get_apps_user_id RETURN NUMBER IS
        l_user_id NUMBER;
    BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            log_msg('Unable to resolve APPS user for apps_initialize: ' || SQLERRM);
            RETURN NULL;
    END get_apps_user_id;

    -- =========================================================================
    -- Helper: Initialize apps context by application short name
    -- =========================================================================
    PROCEDURE initialize_apps_context (
        p_application_short_name IN VARCHAR2,
        p_resp_name_keyword      IN VARCHAR2,
    ...
    BEGIN
        l_user_id := get_apps_user_id();
        IF l_user_id IS NULL THEN
            log_msg('apps_initialize skipped for ' || p_log_label || ': no active user found');
            RETURN;
        END IF;
        ...
        FROM (
            SELECT frv.responsibility_id,
                   frv.application_id,
                   CASE
                       WHEN p_resp_name_keyword IS NOT NULL
                            AND UPPER(frv.responsibility_name) LIKE '%' || UPPER(p_resp_name_keyword) || '%' THEN 0
                       ELSE 1
                   END priority
            ...
            ORDER BY priority, frv.responsibility_id
        )
        WHERE ROWNUM = 1;
        ...
        log_msg('apps_initialize completed for ' || p_log_label ||
                ': user_id=' || l_user_id || ', resp_id=' || l_resp_id ||
                ', app=' || p_application_short_name);
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            log_msg('No active responsibility found for ' || p_log_label ||
                    ' (application=' || p_application_short_name || ')');
        WHEN OTHERS THEN
            log_msg('apps_initialize failed for ' || p_log_label || ': ' || SQLERRM);
    END initialize_apps_context;

    -- =========================================================================
    -- Helper: Call FND_FLEX_EXT.GET_CCID with NLS setup and retry logic
    -- =========================================================================
    FUNCTION get_ccid_with_context (
    ...
    BEGIN
        BEGIN
            ...
            log_msg('NLS language set to AMERICAN before GET_CCID');
        EXCEPTION
            WHEN OTHERS THEN
                log_msg('ALTER SESSION SET NLS_LANGUAGE failed: ' || SQLERRM);
        END;

        log_msg('Looking for CCID: ' || p_concatenated_segments);
        ...
        IF NVL(l_new_ccid, 0) = 0 THEN
            log_msg('Initial FND_FLEX_EXT.GET_CCID failed: ' || fnd_flex_ext.get_message);
            initialize_apps_context(
                p_application_short_name => 'SQLGL',
                p_resp_name_keyword      => 'GENERAL LEDGER',
                p_log_label              => 'GL responsibility'
            );
    ...
    EXCEPTION
        WHEN OTHERS THEN
            log_msg('get_ccid_with_context failed: ' || SQLERRM);
            RETURN 0;
    END get_ccid_with_context;

    -- =========================================================================
    -- Derive CCID from 8-segment combination with dynamic creation
    -- =========================================================================
    FUNCTION derive_ccid (
        p_segment1     IN VARCHAR2,  -- Entity (from AR interface)
        p_segment2     IN VARCHAR2,  -- Division (from AR interface)
        p_segment3     IN VARCHAR2,  -- Account segment (from cogs_account or material_account)
        p_segment4     IN VARCHAR2,  -- Department (from AR interface)
        p_segment5     IN VARCHAR2,  -- Product code (from AR interface)
        p_segment6     IN VARCHAR2,  -- Transaction type (from AR interface)
        p_segment7     IN VARCHAR2,  -- Future1 (from AR interface)
        p_segment8     IN VARCHAR2   -- Future2 (from AR interface)
    ) RETURN NUMBER IS
        l_ccid           NUMBER;
        l_seg1_entity    VARCHAR2(25) := NVL(p_segment1, '01');
        l_seg2_division  VARCHAR2(25) := NVL(p_segment2, '00');
        l_seg3_account   VARCHAR2(25) := p_segment3;
        l_seg4_dept      VARCHAR2(25) := NVL(p_segment4, '00000');
        l_seg5_product   VARCHAR2(25) := NVL(p_segment5, '00000000');
        l_seg6_txn       VARCHAR2(25) := NVL(p_segment6, '0');
        l_seg7_future1   VARCHAR2(25) := NVL(p_segment7, '000');
        l_seg8_future2   VARCHAR2(25) := NVL(p_segment8, '000');
        l_concatenated   VARCHAR2(240);
    BEGIN

        log_msg('  Searching CCID for: ' || l_seg1_entity || '.' || l_seg2_division || '.' ||
        ...
        BEGIN
            ...
            log_msg('  CCID found: ' || l_ccid);
            RETURN l_ccid;
        EXCEPTION
            WHEN no_data_found THEN
                -- CCID not found: attempt to create it via Flexfield API
                log_msg('  CCID not found in GL_CODE_COMBINATIONS — attempting dynamic creation via FND_FLEX_EXT.GET_CCID');

                l_concatenated := l_seg1_entity || '.' || l_seg2_division || '.' ||
                                  l_seg3_account || '.' || l_seg4_dept || '.' ||
                                  l_seg5_product || '.' || l_seg6_txn || '.' ||
                                  l_seg7_future1 || '.' || l_seg8_future2;

                l_ccid := get_ccid_with_context(
                    p_structure_number      => c_chart_of_accounts_id,
                    p_concatenated_segments => l_concatenated
                );

                IF NVL(l_ccid, 0) = 0 THEN
                    log_msg('  FND_FLEX_EXT.GET_CCID failed: ' || fnd_flex_ext.get_message);
                    RETURN -1;
                END IF;

                log_msg('  New CCID created dynamically: ' || l_ccid);
                RETURN l_ccid;
        END;
    EXCEPTION
        WHEN OTHERS THEN
            log_msg('  FATAL error deriving CCID: ' || sqlerrm);
            RETURN -1;
    END derive_ccid;
    ...
    PROCEDURE run_interface (
        ...
        CURSOR c_ar_lines IS
            SELECT
                ...
                ril.org_id,
                ril.uom_code,
                -- Get ALL segments from distributions table
                rid.segment1 AS segment1,
                rid.segment2 AS segment2,
                rid.segment4 AS segment4,
                rid.segment5 AS product_code,
                rid.segment6 AS segment6,
                rid.segment7 AS segment7,
                rid.segment8 AS segment8
            FROM
            ...
            WHERE
                ...
                -- Exclude ASD (Additional Security Deposit) lines and null segments
                AND rid.segment5 IS NOT NULL
                AND NOT (rid.segment5 = '00000000' AND (ril.description LIKE 'ASD%' OR ril.description LIKE 'D-Additional Security Deposit%'))
                -- Date filters
    ...
    BEGIN
        ...
        FOR r IN c_ar_lines LOOP
            BEGIN
                ...
                -- STEP 5: Derive CCIDs using segments from AR interface
                l_debit_ccid := derive_ccid(
                    p_segment1 => r.segment1,
                    p_segment2 => r.segment2,
                    p_segment3 => l_cogs_cogs_account,
                    p_segment4 => r.segment4,
                    p_segment5 => l_product_code,
                    p_segment6 => r.segment6,
                    p_segment7 => r.segment7,
                    p_segment8 => r.segment8
                );
                ...
                -- reference1 = run_id so DR and CR lines share the same journal header
                INSERT INTO apps.gl_interface (
                    status, ledger_id, set_of_books_id, accounting_date, currency_code,
                    ...
                    'NEW', l_sob_id, l_sob_id, l_acct_date, c_currency_code,
                    l_sysdate, c_created_by, c_actual_flag,
                    c_gl_source, c_je_category, l_prd_name,
                    l_total_cost, l_total_cost, l_dr_ccid,
                    r.segment1, r.segment2, l_cogs_acct, r.segment4, l_prod_code, r.segment6, r.segment7, r.segment8,
                    l_run_id,                     -- reference1: batch name
                    'COGS Journal ' || l_prd_name, -- reference2: header name
                    'COGS ' || l_prd_name,        -- reference4: SAME for all lines = one header
                    l_order_number,               -- reference5: order number
                    TO_CHAR(l_unit_cost),         -- reference6: unit cost
                    l_prod_code || '-' || NVL(r.uom_code, l_cogs_uom) || '-' || TO_CHAR(l_qty), -- reference10 -> description
                    NVL(r.uom_code, l_cogs_uom),  -- reference29 -> gl_je_lines.reference_9: UOM
                    l_qty,                        -- stat_amount: quantity
                    l_prod_code,
                    l_cogs_acct,
                    l_mat_acct,
                ...
                INSERT INTO apps.gl_interface (
                    ...
                    r.segment1, r.segment2, l_mat_acct, r.segment4, l_prod_code, r.segment6, r.segment7, r.segment8,
                    l_run_id,                     -- reference1: batch name (same as DR)
                    'COGS Journal ' || l_prd_name, -- reference2: header name
                    'COGS ' || l_prd_name,        -- reference4: SAME for all lines = one header
                    l_order_number,               -- reference5: order number
                    TO_CHAR(l_unit_cost),         -- reference6: unit cost
